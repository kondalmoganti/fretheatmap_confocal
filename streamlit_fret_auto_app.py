...
with tabs[0]:
    ...
    if not mats:
        st.info("No matrix-like block found in this file.")
    else:
        iM = st.selectbox(
            "Choose matrix block",
            mats,
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            zmin = st.number_input("zmin (0=auto)", value=0.0, key="hm_zmin_best")
        with c2:
            zmax = st.number_input("zmax (0=auto)", value=0.0, key="hm_zmax_best")
        with c3:
            cmap = st.selectbox("Colorscale", colorscales, index=0, key="hm_cmap_best")
        smooth = st.slider("Gaussian smoothing (σ)", 0.0, 6.0, 1.0, 0.1, key="hm_smooth_best")
        arr = dfm.to_numpy()
        arrp = (
            gaussian_filter1d(gaussian_filter1d(arr, sigma=smooth, axis=0), sigma=smooth, axis=1)
            if smooth > 0
            else arr
        )
        fig = px.imshow(
        ...
        st.plotly_chart(fig, use_container_width=True, key="fig_heatmap")
...
with tabs[1]:
    ...
    if not tbls:
        ...
        iT = st.selectbox(
            ...
            tbls,
            index=tbls[-1],
            key="single_block_best",
            format_func=lambda i: f"Block {i} (shape {blocks[i][0].shape})",
        )
        dft = blocks[iT][0].copy()
        templ = st.radio(
            "Column template", ["Generic (C0..)", "FRET 8-cols (named)"], index=1, key="single_template_best"
        )
        ...
        with c1:
            x_col = st.selectbox("Values column (x)", dft.columns, index=0, key="single_xcol_best")
        with c2:
            w_col = st.selectbox(
                "Weights (optional)", ["(none)"] + list(dft.columns), index=0, key="single_wcol_best"
            )
        with c3:
            bin_mode = st.radio("Binning", ["Auto (rule)", "Manual"], index=0, key="single_binmode_best")
        if bin_mode == "Auto (rule)":
            with c4:
                rule = st.selectbox(
                    "Auto-binning rule", ["Freedman–Diaconis", "Scott", "Sturges"], index=0, key="single_rule_best"
                )
            edges = None
        else:
            edges, centers_dummy = clamp_manual_bins_E(
                "Range min", "Range max", "Bins (manual)", default_bins=80, key_prefix="single"
            )
        ...
        if not np.any(m):
            ...
            st.plotly_chart(figH, use_container_width=True, key="fig_hist_single")
...
with tabs[2]:
    ...
    if not cand:
        ...
        if bin_mode == "Auto (rule)":
            rule = st.selectbox(
                "Auto-binning rule", ["Freedman–Diaconis", "Scott", "Sturges"], index=0, key="ov_rule_best"
            )
            edges = None
        else:
            edges, _ = clamp_manual_bins_E(
                "Range min", "Range max", "Bins (manual)", default_bins=80, key_prefix="ov"
            )
        ...
        if mode == "Stoichiometry S":
            ...
            xlabel = "Stoichiometry [S]"
            legends = ("Classical (S)", "PIE (S)")
        else:
                ...
                if fit_cl:
                    if fit_cl["model"] == "1G":
                        fig.add_trace(
                            go.Scatter(
                                x=xs,
                                y=gaussian(xs, *fit_cl["params"]),
                                mode="lines",
                                name=f"{legends[0]} fit 1G (R²={fit_cl['R2']:.3f})",
                            )
                        )
                    else:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                x=xs,
                                y=gaussian2(xs, *fit_cl["params"]),
                                mode="lines",
                                name=f"{legends[0]} fit 2G (R²={fit_cl['R2']:.3f})",
                            )
                        )
                        fig.add_trace(
                            go.Scatter(
                ...
                if fit_pie:
                    if fit_pie["model"] == "1G":
                        fig.add_trace(
                            go.Scatter(
                                x=xs,
                                y=gaussian(xs, *fit_pie["params"]),
                                mode="lines",
                                name=f"{legends[1]} fit 1G (R²={fit_pie['R2']:.3f})",
                            )
                        )
                    else:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                x=xs,
                                y=gaussian2(xs, *fit_pie["params"]),
                                mode="lines",
                                name=f"{legends[1]} fit 2G (R²={fit_pie['R2']:.3f})",
                            )
                        )
                        fig.add_trace(
                            go.Scatter(
...
with tabs[5]:
    ...
    if files:
        source = st.radio(
            "E source to compare", ["PIE", "Classical"], index=0, horizontal=True, key="multi_source_best"
        )
        normalize = st.checkbox("Normalize each histogram area to 1", value=True, key="multi_norm_best")

        bin_mode = st.radio(
            "Binning", ["Auto (rule)", "Manual"], index=0, horizontal=True, key="multi_binmode_best"
        )
        if bin_mode == "Auto (rule)":
            rule = st.selectbox(
                "Auto-binning rule (shared)", ["Freedman–Diaconis", "Scott", "Sturges"], index=0, key="multi_rule_best"
            )
            edges = None
        else:
            edges, _ = clamp_manual_bins_E(
                "E min (shared)", "E max (shared)", "Bins (manual, shared)", default_bins=80, key_prefix="multi"
            )
        ...
        if len(all_E) == 0:
            ...
            fig.add_vrect(
                x0=region[0], x1=region[1], fillcolor="LightSalmon", opacity=0.2, layer="below", line_width=0
            )
            fig.update_layout(
            ...
            st.plotly_chart(fig, use_container_width=True, key="fig_multi_overlay")

            df_sum = pd.DataFrame(summary)
            st.subheader("Population in selected region")
            st.dataframe(df_sum, use_container_width=True, key="tbl_multi_summary")
            st.download_button(
